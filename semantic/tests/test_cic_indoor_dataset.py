...
from supervision.utils import ColorPalette
...
def main():
    ...
    if ANNOTATE:
        ...
        ANNOTATE_OUT_DIR = os.path.join('debug_mapper', 'annotated_3d_in_loop')
    ...
    mapper = ObjMapper(tracker=tracker, label_template=label_template, visualize=VISUALIZE)

    global_cloud = np.empty((0, 3))

    image_stamps = image_stamps[:4000:5]
    for cnt, image_stamp in enumerate(tqdm(image_stamps)):
        ...
        detections_tracked = mapper.track_objects(det_bboxes, det_labels, det_confidences, odom)
        ...
        if VISUALIZE:
            # if cnt % 5 == 0:
            mapper.rerun_vis(odom, regularized=True, show_bbox=True, debug=False)

            map_dict = mapper.serialize_map_to_dict(image_stamp)
            import json
            with open(os.path.join('output/json_serialization', f"{image_stamp}.json"), 'w') as f:
        ...
        if ANNOTATE:
            ...
            generate_seg_cloud(cloud_body, masks, labels, confidences, R_b2w, t_b2w, platform='diablo', image_src=image_anno)
