...
class VisualizerRerun(VisualizerBase):
    ...
    #     )
    def visualize_global_pcd(self, pcd_numpy):
        if pcd_numpy is None or pcd_numpy.size == 0:
            return
            
        num_points = pcd_numpy.shape[0]
        
        # 使用较暗的灰色，让它作为背景，突出彩色的语义物体
        colors = np.full((num_points, 3), [100, 100, 100], dtype=np.uint8) 
        
        rr.log(
            "world/scene_map", # 使用唯一的路径
            rr.Points3D(
                pcd_numpy,
                colors=colors,
                radii=0.01, # 点可以设小一点，看起来更致密
            ),
        )
