        ...
        if self.captioner is not None and image is not None:
            ...
            for i, obj_id in enumerate(obj_ids):
                ...
                if final_w <= 0 or final_h <= 0:
                    # self.log_info(f"警告：裁剪后物体 ID {obj_id} 尺寸为0，跳过处理")
                    continue
                
                # 即使很大，如果只有 1-2 像素，CLIP 也会报错或效果极差
                if final_w < 3 or final_h < 3:
                    continue
                    
                clipped_bbox = [ix1, iy1, ix2, iy2]
                # ----------------------------------
                ...
                for single_obj in self.single_obj_list:
                    if obj_id in single_obj.obj_id:
                        ...
                        if cent_3d is not None:
                            ...
                            bboxes_2d.append(clipped_bbox) # 使用裁剪后的整数框
            ...
            if bboxes_2d:
                ...
                self.captioner.update_object_crops(
                    ...
                    bboxes_3d=bboxes_3d,
                    image_anno=[None] * len(obj_ids_updated)
                )
