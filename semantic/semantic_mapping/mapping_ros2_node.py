...
class MappingNode(Node):
    ...
    def handle_object_query(self, query_str: String):
        ...
        # 1. 生成描述 (这步很慢)
        result = self.captioner.query_clip_features(query_list, self.cur_pos, self.cur_orient)
        self.queried_captions = result
        
        # 2. 【关键修改】不靠定时器，直接在这里发布
        try:
    ...
    def generate_freespace(self, msg: PointCloud2):
        ...
        if self.cur_pos_for_freespace is None or np.linalg.norm(self.cur_pos[:2] - self.cur_pos_for_freespace[:2]) > self.pos_change_threshold:
            ...
            # 2. 提取地面点 (Z < 0.2)
            ground_mask = full_pcd_np[:, 2] < 0.2
            new_ground_points = full_pcd_np[ground_mask]
            
            # 3. 提取障碍物点 (0.2 <= Z < 2.0，过滤掉天花板)
            obstacle_mask = (full_pcd_np[:, 2] >= 0.2) & (full_pcd_np[:, 2] < 2.0)
            new_obstacle_points = full_pcd_np[obstacle_mask]
            
            # --- 处理地面累积 ---
            if self.freespace_pcl is None:
                self.freespace_pcl = new_ground_points
            else:
                self.freespace_pcl = np.vstack([self.freespace_pcl, new_ground_points])
            
            # --- 处理障碍物累积 (map_pcl) ---
            if not hasattr(self, 'map_pcl') or self.map_pcl is None:
                self.map_pcl = new_obstacle_points
            else:
                self.map_pcl = np.vstack([self.map_pcl, new_obstacle_points])
            
            # 4. 执行下采样 (地面: 0.05m 精度)
            f_pcd = o3d.geometry.PointCloud()
            f_pcd.points = o3d.utility.Vector3dVector(self.freespace_pcl)
            f_pcd = f_pcd.voxel_down_sample(voxel_size=0.05)
            self.freespace_pcl = np.asarray(f_pcd.points)

            # 5. 执行下采样 (障碍物: 0.1m 精度，略低以提升推理效率)
            m_pcd = o3d.geometry.PointCloud()
            m_pcd.points = o3d.utility.Vector3dVector(self.map_pcl)
            m_pcd = m_pcd.voxel_down_sample(voxel_size=0.1)
            self.map_pcl = np.asarray(m_pcd.points)

            # 6. 持久化保存到磁盘 (使用绝对路径)
            save_dir = "/home/iot/hm/ros2_ws/maps"
            if not os.path.exists(save_dir):
                os.makedirs(save_dir, exist_ok=True)
            
            try:
                # 保存地面文件
                f_path = os.path.join(save_dir, "freespace_map.ply")
                o3d.io.write_point_cloud(f_path, f_pcd)
        ...
        # 7. 发布实时 freespace 话题 (供 Planner 使用)
        if self.freespace_pcl is not None:
