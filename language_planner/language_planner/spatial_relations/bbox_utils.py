...
def calculate_iom_single(bbox1, bbox2):
    # determine the coordinates of intersection rectangle
    xA = np.maximum(bbox1[0], bbox2[0])
    yA = np.maximum(bbox1[1], bbox2[1])
    xB = np.minimum(bbox1[2], bbox2[2])
...
def calculate_iom_poly(bbox1, bbox2):
    '''
    Calculate intersection over minimum area between two oriented 2D bounding boxes.
    '''
    bbox1_2d = bbox1[[0, 3, 2, 1, 0], :2]
    bbox2_2d = bbox2[[0, 3, 2, 1, 0], :2]

    poly1 = Polygon(bbox1_2d)
    poly2 = Polygon(bbox2_2d)
...
def get_bbox_coords_heading_xyzlwh(centroid, dimensions, heading):
